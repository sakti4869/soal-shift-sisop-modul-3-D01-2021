...
#define FILES_MAX 4 // Jumlah thread buat proses file max
...
void* moveFile(void* arg)
{
	...
	if (tipeFile == 0) { // Kalo filenya file biasa
		...
		char fileNameNewPath[128]; // Nama file dengan folder baru
...
void* moveDir(void* arg) {
	...
	char fileWithPath[128]; // Nama file tambah pathnya
	...
	char fileNameList[FILES_MAX][128]; // Nama - nama file buat dikategorikan
	...
	while ((dent = readdir(dir)) != NULL) {
		if (strcmp(dent->d_name, "..") && strcmp(dent->d_name, ".")) {
			...
			if (!isDirectory(fileWithPath)) {
				strcpy(fileNameList[fileIndex], fileWithPath);
				fileIndex++;
			}
		}

		if (fileIndex == FILES_MAX) { // Kalau udah cukup file buat dikategorikan
			int j;
			for (j = 0; j < fileIndex; j++) {
